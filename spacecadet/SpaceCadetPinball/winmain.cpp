...
int winmain::WinMain(LPCSTR lpCmdLine)
{
	...
	// PB init from message handler
	{
		...
		int soundInit = Sound::Init(voiceCount);
		printf("winmain::WinMain Sound::Init returned %d (0=success)\n", soundInit);
		if (soundInit)
			options::Options.Sounds = 0;
		Sound::Activate();
		printf("winmain::WinMain After Activate Options.Sounds=%d\n", options::Options.Sounds);

		if (!pinball::quickFlag && !midi::music_init())
			options::Options.Music = 0;
		printf("winmain::WinMain Music init Options.Music=%d\n", options::Options.Music);
